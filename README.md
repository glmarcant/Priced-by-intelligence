...
This repository contains the code only. The discussion and interpretation of the results are in the thesis itself, included in this repository as `thesis.pdf`.
...
| `robustness_aapl.ipynb`, `robustness_spx.ipynb` | Robustness check for Black-Scholes and XGBoost on three alternative non-overlapping chronological splits, reusing the hyperparameters of the main analysis |
| `robustness_nn_aapl.ipynb`, `robustness_nn_spx.ipynb` | Same robustness check for the neural network |
| `cv_results_v4_spx.csv` | Cross-validation results for the main SPX XGBoost specification |
| `thesis.pdf` | Full text of the thesis, with the discussion of all results |
| `*.png` | Figures used in the thesis |
...
5. `extrapolation_chart_aapl.ipynb`
6. `robustness_aapl.ipynb`, `robustness_spx.ipynb`, `robustness_nn_aapl.ipynb` and `robustness_nn_spx.ipynb`
